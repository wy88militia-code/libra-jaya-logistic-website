...
const INCIDENTS=new Set(['HELD','DAMAGED','CLAIM_PROCESS']);
...
function scanMatchesBooking(scanCode,booking){if(!scanCode)return false;const candidates=[booking.bookingId,booking.partnerReference,booking.awb,booking.trackingNumber].filter(Boolean).map(normalizeScan);return candidates.includes(normalizeScan(scanCode));}

export async function savePodFile(file,bookingId){
  const id=clean(bookingId,120);const booking=await getBooking(id);if(!booking)throw new Error('Booking tidak ditemukan.');
  if(booking.status==='DELIVERED'||booking.podId)throw new Error('POD sudah dikunci dan tidak dapat diganti setelah DELIVERED.');
  ...
  const podId=`POD-${Date.now()}-${crypto.randomBytes(5).toString('hex')}`;const bytes=Buffer.from(await file.arrayBuffer());const digest=sha256(bytes);
  await podStore().setJSON(`pod/${podId}`,{podId,bookingId:id,contentType:file.type,originalName:clean(file.name||'pod',120),size:file.size,sha256:digest,base64:bytes.toString('base64'),uploadedAt:now(),immutable:true},{onlyIfNew:true});return podId;
}
...
export async function addTrackingEvent(input={}){
  ...
  const note=clean(input.note,1000),receiverName=clean(input.receiverName,120),podId=clean(input.podId,140),scanCode=clean(input.scanCode,180);
  if(SCAN_REQUIRED.has(status)){if(!scanCode)throw new Error(`Status ${status} wajib scan AWB/QR.`);if(!scanMatchesBooking(scanCode,booking))throw new Error('AWB/QR yang dipindai tidak cocok dengan Booking ID / referensi partner.');}
  ...
  const patch={status,currentTrackingStatus:status,lastTrackingAt:createdAt,lastTrackingEventHash:event.eventHash};
  ...
  if(INCIDENTS.has(status)){patch.hasIncident=true;patch.incidentStatus=status;patch.incidentNote=note;patch.claimStatus=event.claimStatus;patch.claimReference=event.claimReference;}
