...
export async function authenticateApiRequest(request){
  ...
  return {partner,bodyText,json,url,method:request.method.toUpperCase(),nonce,timestamp,environment,lifecycle,credentialEnvironment:resolved.credentialEnvironment,requestId:`API-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`};
}
