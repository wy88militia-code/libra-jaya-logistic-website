...
function privacyFingerprint(value,label){const raw=String(value||'').trim(),secret=String(process.env.API_LOG_PRIVACY_SECRET||process.env.PARTNER_SESSION_SECRET||process.env.ADMIN_SESSION_SECRET||'');if(!raw||secret.length<32)return null;return crypto.createHmac('sha256',secret).update(`${label}|${raw}`).digest('base64url').slice(0,32);}
...
export async function writeApiLog(context,{status,action,reference=null,error=null}={}){
  if(!context?.partner)return;const createdAt=now();const numericStatus=Number(status)||0;const queryKeys=context.url?[...new Set([...context.url.searchParams.keys()])].sort().slice(0,30):[];const log={requestId:context.requestId,partnerId:context.partner.partnerId,environment:context.environment||'PRODUCTION',method:context.method,path:context.url?.pathname||null,queryKeys,action:action||null,status:numericStatus,reference,error:error?String(error).slice(0,500):null,clientIpHash:privacyFingerprint(context.clientIp,'ip'),createdAt};await getStore(LOG_STORE).setJSON(`log/${context.partner.partnerId}/${createdAt}-${context.requestId}`,log,{onlyIfNew:true});
