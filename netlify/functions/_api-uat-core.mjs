...
export async function buildUatEvidence(partnerId){
  ...
  const checks={
    ...
    serviceability:pass(serviceabilitySuccess,serviceabilitySuccess?'GET /api/v1/serviceability berhasil membaca coverage/rate plan.':'Serviceability UAT belum berhasil.'),
  ...
  return {partnerId:id,record,logs:logs.slice(0,100),checks,baselineVerdict,stats:{uatRequests:logs.length,success:successful.length,serviceabilityChecks:logs.filter(x=>x.action==='SERVICEABILITY').length,clientErrors:logs.filter(x=>x.status>=400&&x.status<500).length,serverErrors:serverErrors.length},wallet:{balance:Number(wallet.balance)||0,requiredDeposit,depositReady},stage};
}
