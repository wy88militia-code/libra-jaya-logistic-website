...
import { getAssignment } from './_courier-custody-core.mjs';
...
const OUTBOUND_TARGETS=new Set(['COURIER','MANIFEST']);
...
async function validateInboundCustody(bookingId){const assignment=await getAssignment(bookingId);if(!assignment)return null;if(assignment.pendingHandoverTo)throw new Error('Inbound gudang diblokir karena handover custody masih pending.');if(assignment.currentCustodian)throw new Error(`Inbound gudang diblokir: barang masih dalam custody ${assignment.currentCustodian}. Release at Hub terlebih dahulu.`);if(!['AT_HUB','ASSIGNED','COMPLETED'].includes(String(assignment.status||'')))throw new Error(`Status custody ${assignment.status||'-'} belum konsisten untuk inbound gudang.`);return assignment;}
async function validateOutboundTarget(bookingId,targetType,targetRef){if(!OUTBOUND_TARGETS.has(targetType))throw new Error('OUTBOUND wajib memilih target COURIER atau MANIFEST.');if(targetType==='COURIER'){const assignment=await getAssignment(bookingId);if(!assignment)throw new Error('OUTBOUND ke kurir membutuhkan courier assignment.');if(assignment.pendingHandoverTo)throw new Error('OUTBOUND ke kurir diblokir karena handover custody masih pending.');if(!assignment.currentCustodian)throw new Error('Kurir harus TAKE_CUSTODY terlebih dahulu sebelum barang di-scan OUTBOUND dari gudang.');if(targetRef&&String(assignment.currentCustodian).toLowerCase()!==String(targetRef).toLowerCase())throw new Error(`Custodian aktif ${assignment.currentCustodian} tidak sama dengan target kurir ${targetRef}.`);return {targetType,targetRef:assignment.currentCustodian};}
 const manifestId=clean(targetRef,140);if(!manifestId)throw new Error('Manifest ID wajib untuk OUTBOUND ke manifest.');const manifest=await getManifest(manifestId);if(!manifest)throw new Error('Manifest tidak ditemukan.');const item=manifest.items?.[bookingId];if(!item)throw new Error('Booking belum terdaftar di manifest tersebut.');if(!['CLOSED','DEPARTED'].includes(manifest.status))throw new Error(`Manifest harus CLOSED/DEPARTED sebelum outbound gudang, status saat ini ${manifest.status}.`);const uplift=Number(item.upliftWeightKg||0),offload=Number(item.offloadWeightKg||0);if(uplift-offload<=0)throw new Error('Booking belum memiliki net uplift pada manifest. Catat uplift terlebih dahulu.');return {targetType,targetRef:manifestId};}
export async function warehouseAction(input={},actor='admin'){
 ...
 if(action==='INBOUND')await validateInboundCustody(bookingId);
 let outboundTarget=null;if(action==='OUTBOUND')outboundTarget=await validateOutboundTarget(bookingId,upper(input.targetType,20),clean(input.targetRef,140));
 const at=now(),evidence=scanEvidence(input.scanCode);let next;
 if(action==='INBOUND')next={bookingId,status:'INBOUND',hub,zone:null,rack:null,condition,createdAt:current?.createdAt||at,createdBy:current?.createdBy||clean(actor,100),updatedAt:at,updatedBy:clean(actor,100),lastScanHash:evidence.scanHash,lastScanLast4:evidence.scanLast4,lastOutboundTargetType:null,lastOutboundTargetRef:null};
 else next={...current,status:action,hub,zone:action==='OUTBOUND'?null:zone||current.zone||null,rack:action==='OUTBOUND'?null:rack||current.rack||null,condition,updatedAt:at,updatedBy:clean(actor,100),lastScanHash:evidence.scanHash,lastScanLast4:evidence.scanLast4,outboundAt:action==='OUTBOUND'?at:current.outboundAt||null,lastOutboundTargetType:action==='OUTBOUND'?outboundTarget.targetType:current.lastOutboundTargetType||null,lastOutboundTargetRef:action==='OUTBOUND'?outboundTarget.targetRef:current.lastOutboundTargetRef||null};
 const saved=await saveCas(entry,next);await appendEvent(saved,{type:action,actor,hub,zone,rack,condition,note:input.note,targetType:outboundTarget?.targetType,targetRef:outboundTarget?.targetRef,...evidence});return saved;
}
