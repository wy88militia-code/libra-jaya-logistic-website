...
function render(result=null,error=''){
  ...
  const detailRows=(result?.results||[]).map(r=>`<tr><td><b class="${r.status==='POSTED'?'ok':['RECONCILE_REQUIRED','POST_FAILED','EXCEPTION','RETRY'].includes(r.status)?'bad':'warn'}">${esc(r.status||'-')}</b></td><td>${esc(r.source||'-')}<small>${esc(r.partnerId||'-')}</small></td><td>${rp(r.amount)}<small>${esc(r.reference||'-')}</small></td><td>${esc(r.journalNumber||'-')}</td><td>${esc(r.reason||'-')}${r.attempts?`<small>Attempts: ${Number(r.attempts)}</small>`:''}</td></tr>`).join('');
  return `<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Force Accurate Worker | Libra</title><style>*{box-sizing:border-box}body{margin:0;background:#f2f6fa;color:#10243d;font-family:Inter,system-ui}.top{background:#061d36;color:#fff;padding:18px}.topin,.wrap{max-width:760px;margin:auto}.topin{display:flex;justify-content:space-between;gap:12px}.top a{color:#fff}.wrap{padding:22px 16px}.card{background:#fff;border:1px solid #dce6ef;border-radius:18px;padding:20px;margin-bottom:14px}.ok{color:#176b37}.warn{color:#8b6511}.bad{color:#a12e27}.btn{border:0;border-radius:12px;background:#0b4d7a;color:#fff;padding:14px 18px;font-weight:850;font-size:16px}.note{background:#eef5fb;border-left:5px solid #0b4d7a}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap;word-break:break-word}.tablewrap{overflow:auto}table{width:100%;min-width:680px;border-collapse:collapse;font-size:12px}th,td{padding:9px;border-bottom:1px solid #edf1f5;text-align:left;vertical-align:top}th{background:#f7fafc}small{display:block;color:#718397;margin-top:3px}</style></head><body><header class="top"><div class="topin"><strong>LIBRA • Accurate Worker</strong><a href="/admin-accurate/auto">← Accurate Auto</a></div></header><main class="wrap"><section class="card"><h1>Force Worker Sekarang</h1><p>Menjalankan satu siklus worker yang sama dengan jadwal 5-menit. Tidak ada bypass production gate, database identity, COA, cabang, duplicate guard, atau read-back verification.</p><form method="post"><button class="btn" type="submit">Jalankan 1 Siklus Sekarang</button></form></section><section class="card"><h2>Status: <span class="${cls}">${esc(status)}</span></h2>${error?`<p class="bad">${esc(error)}</p>`:''}${result?`<p><b>Processed:</b> ${Number(result.processed||0)}<br><b>Skipped:</b> ${result.skipped?'YA':'TIDAK'}<br><b>Reason:</b> ${esc(result.reason||'-')}</p><div class="mono">${esc(JSON.stringify(counts,null,2))}</div>`:'<p>Belum dijalankan manual.</p>'}</section>${detailRows?`<section class="card"><h2>Detail Hasil Worker</h2><div class="tablewrap"><table><thead><tr><th>Status</th><th>Sumber / Partner</th><th>Nominal / Referensi</th><th>Journal Voucher</th><th>Alasan / Attempts</th></tr></thead><tbody>${detailRows}</tbody></table></div></section>`:''}<section class="card note"><b>Fail-safe tetap aktif.</b><p>Jika hasil menjadi RECONCILE_REQUIRED atau status posting tidak pasti, jangan jalankan ulang secara buta. Periksa Journal Voucher di Accurate terlebih dahulu.</p></section></main></body></html>`;
}
