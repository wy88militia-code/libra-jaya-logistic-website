...
function num(v){
  ...
  let x=String(v??'').trim().replace(/\s/g,'').replace(/[^0-9,.-]/g,'');if(!x)return null;
  if(x.includes(',')&&x.includes('.')){const c=x.lastIndexOf(','),d=x.lastIndexOf('.');x=c>d?x.replace(/\./g,'').replace(',','.'):x.replace(/,/g,'');}
  else if(x.includes(',')){const p=x.split(',');x=p.length===2&&p[1].length<=2?`${p[0]}.${p[1]}`:p.join('');}
...
async function readRanges(){
  const c=config(),token=await accessToken(),ranges=["'VENDOR_MASTER'!A1:H1000","'VENDOR_RATE'!A1:Q5000","'SURCHARGE'!A1:J2000"],p=new URLSearchParams({majorDimension:'ROWS',valueRenderOption:'UNFORMATTED_VALUE'});
...
function validate(vendors,rates,surcharges){
  const errors=[];if(!vendors.length)errors.push('VENDOR_MASTER belum memiliki vendor.');if(!rates.length)errors.push('VENDOR_RATE belum memiliki rate.');
  const vendorIds=new Set(),activeVendorIds=new Set();for(const v of vendors){if(vendorIds.has(v.vendorId))errors.push(`Vendor ID duplikat: ${v.vendorId}`);vendorIds.add(v.vendorId);if(!v.vendorName)errors.push(`Nama vendor wajib: ${v.vendorId}`);if(v.status==='ACTIVE')activeVendorIds.add(v.vendorId);}
  const rateIds=new Set();for(const r of rates){if(rateIds.has(r.rateId))errors.push(`Rate ID duplikat: ${r.rateId}`);rateIds.add(r.rateId);if(!activeVendorIds.has(r.vendorId))errors.push(`Vendor tidak aktif/tidak ditemukan: ${r.vendorId}`);if(!VALID_CATEGORIES.has(r.category))errors.push(`Kategori rate tidak valid: ${r.rateId}`);if(!VALID_RATE_TYPES.has(r.rateType))errors.push(`Rate type tidak valid: ${r.rateId}`);if(!r.routeCode)errors.push(`Route code wajib: ${r.rateId}`);if(r.currency!=='IDR')errors.push(`Currency sementara wajib IDR: ${r.rateId}`);if(r.minChargeKg<0)errors.push(`Minimum charge negatif: ${r.rateId}`);if(r.rateType==='PER_KG'&&r.ratePerKg<=0)errors.push(`Rate/kg wajib > 0: ${r.rateId}`);if(r.rateType==='FLAT'&&r.flatAmount<=0)errors.push(`Flat amount wajib > 0: ${r.rateId}`);if(r.effectiveFrom&&r.effectiveTo&&r.effectiveTo<r.effectiveFrom)errors.push(`Periode rate terbalik: ${r.rateId}`);}
  const surchargeIds=new Set();for(const s of surcharges){if(surchargeIds.has(s.surchargeId))errors.push(`Surcharge ID duplikat: ${s.surchargeId}`);surchargeIds.add(s.surchargeId);if(!activeVendorIds.has(s.vendorId))errors.push(`Vendor surcharge tidak aktif/tidak ditemukan: ${s.vendorId}`);if(!s.routeCode)errors.push(`Route surcharge wajib: ${s.surchargeId}`);if(s.category&&!VALID_CATEGORIES.has(s.category))errors.push(`Kategori surcharge tidak valid: ${s.surchargeId}`);if(s.amount<0)errors.push(`Surcharge negatif: ${s.surchargeId}`);if(s.effectiveFrom&&s.effectiveTo&&s.effectiveTo<s.effectiveFrom)errors.push(`Periode surcharge terbalik: ${s.surchargeId}`);}
  return errors;
}
async function buildFromSheet(){const [vr,rr,sr]=await readRanges(),vendors=parseVendors(vr?.values),rates=parseRates(rr?.values),surcharges=parseSurcharges(sr?.values),errors=validate(vendors,rates,surcharges);return {vendors,rates,surcharges,errors};}

export async function autoSyncVendorMaster(source='AUTO_SCHEDULED'){
  const {vendors,rates,surcharges,errors}=await buildFromSheet(),s=getStore(STORE),stamp=new Date().toISOString(),c=config();
  if(errors.length){const failed={status:'INVALID',syncedAt:stamp,source,stats:{vendors:vendors.length,rates:rates.length,surcharges:surcharges.length,errors:errors.length},errors:errors.slice(0,200)};await s.setJSON(LAST_SYNC,failed);throw new Error(`Vendor Master invalid: ${errors.slice(0,5).join(' | ')}`);}
  const previous=await s.get(CURRENT,{type:'json',consistency:'strong'}),snapshot={version:crypto.randomUUID(),sheetId:c.sheetId,sheetUrl:`https://docs.google.com/spreadsheets/d/${c.sheetId}/edit`,status:'PUBLISHED',syncMode:'AUTO',syncSource:clean(source,100),publishedAt:stamp,syncedAt:stamp,stats:{vendors:vendors.length,rates:rates.length,surcharges:surcharges.length,errors:0},errors:[],vendors,rates,surcharges};
  if(previous?.version)await s.setJSON(`${HISTORY_PREFIX}${previous.version}`,previous,{onlyIfNew:true}).catch(()=>{});await s.setJSON(CURRENT,snapshot);await s.delete(PENDING).catch(()=>{});await s.setJSON(LAST_SYNC,{status:'OK',version:snapshot.version,syncedAt:stamp,source:snapshot.syncSource,stats:snapshot.stats});return snapshot;
}
export async function getLastVendorSync(){return getStore(STORE).get(LAST_SYNC,{type:'json',consistency:'strong'});}
export async function previewVendorMaster(actor='finance'){const {vendors,rates,surcharges,errors}=await buildFromSheet(),c=config(),snapshot={version:crypto.randomUUID(),sheetId:c.sheetId,sheetUrl:`https://docs.google.com/spreadsheets/d/${c.sheetId}/edit`,status:errors.length?'INVALID':'PENDING_APPROVAL',previewedAt:new Date().toISOString(),previewedBy:clean(actor,100),stats:{vendors:vendors.length,rates:rates.length,surcharges:surcharges.length,errors:errors.length},errors:errors.slice(0,200),vendors,rates,surcharges};await getStore(STORE).setJSON(PENDING,snapshot);return snapshot;}
export async function publishVendorMaster(actor='superadmin'){const s=getStore(STORE),pending=await s.get(PENDING,{type:'json',consistency:'strong'});if(!pending)throw new Error('Tidak ada preview Vendor Master.');if(pending.status==='INVALID'||pending.errors?.length)throw new Error('Vendor Master masih memiliki error dan tidak dapat dipublish.');const publisher=clean(actor,100);if(pending.previewedBy&&pending.previewedBy===publisher)throw new Error('Maker dan checker Vendor Master harus user yang berbeda.');const published={...pending,status:'PUBLISHED',publishedAt:new Date().toISOString(),publishedBy:publisher};const history=await s.setJSON(`${HISTORY_PREFIX}${published.version}`,published,{onlyIfNew:true});if(!history.modified)throw new Error('Snapshot version sudah pernah dipublish.');await s.setJSON(CURRENT,published);await s.delete(PENDING);return published;}
...
export async function estimateVendorCostForBooking(booking={},snapshot=null){
  ...
  const byCategory=new Map();for(const r of candidates)if(!byCategory.has(r.category))byCategory.set(r.category,r);const components=[];
