...
export async function getVerifiedOperationalWeight(bookingId){const r=await getWeightRecord(bookingId);if(!r||r.status!=='VERIFIED')return null;return {bookingId:r.bookingId,actualWeightKg:r.actualWeightKg,volumetricWeightKg:r.volumetricWeightKg,chargeableWeightKg:r.chargeableWeightKg,packageCount:r.packageCount,verifiedAt:r.updatedAt,reweighCount:r.reweighCount,billingReviewRequired:Boolean(r.billingReviewRequired),billingReviewStatus:r.billingReviewStatus||'NONE'};}
