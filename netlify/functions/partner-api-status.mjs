...
export default async request=>{
  ...
  try{
    ...
    const ai=evidence.record.aiAnalysis;
    return Response.json({ok:true,partner:{partnerId:partner.partnerId,companyName:partner.companyName,picName:partner.picName,apiKeyMasked:maskedKey(partner.apiKey),portalActivated:Boolean(partner.portalActivated),portalActivatedAt:partner.portalActivatedAt||null},onboarding:{applicationId:partner.onboardingApplicationId,status:application?.status||null,callbackUrl:application?.callbackUrl||null},uat:{stage:evidence.stage,baselineVerdict:evidence.baselineVerdict,finalDecision:evidence.record.finalDecision||'PENDING',productionEnabled:Boolean(evidence.record.productionEnabled),checks:safeChecks(evidence.checks),webhookStatus:evidence.record.webhookStatus||'NOT_TESTED',webhookNote:evidence.record.webhookNote||'',lastWebhookTestAt:evidence.record.lastWebhookTestAt||null,aiAnalysis:ai?{verdict:ai.verdict,confidence:ai.confidence,summary:ai.summary,blockers:Array.isArray(ai.blockers)?ai.blockers:[],nextActions:Array.isArray(ai.next_actions)?ai.next_actions:[],analyzedAt:ai.analyzedAt||null}:null},wallet:evidence.wallet,webhookDeliveries:deliveries.map(d=>({deliveryId:d.deliveryId,createdAt:d.createdAt,eventType:d.eventType,environment:d.environment,status:d.status,attempts:d.attempts,maxAttempts:d.maxAttempts,lastHttpStatus:d.lastHttpStatus,lastError:d.lastError,deliveredAt:d.deliveredAt}))});
