...
export default async request=>{
  ...
  try{
    ...
    const quoteId=String(context.json?.quoteId||context.json?.quote_id||'').trim();const quote=quoteId?await getQuote(quoteId):null;const amount=quote?.partnerId===context.partner.partnerId?Number(quote.amount)||0:0;await consumeBookingQuota(context.partner.partnerId,context.environment,context.policy,amount);
    const uat=context.environment==='UAT';const result=uat?await createUatBookingForPartner(context.partner,context.json||{},idem):await createBookingForPartner(context.partner,context.json||{},idem,'API');if(result.duplicate)await recordDuplicateBookingSignal(context.partner.partnerId,context.environment,context.policy);await writeApiLog(context,{status:result.duplicate?200:201,action:uat?'BOOKING_UAT':'BOOKING',reference:result.booking.bookingId});
    return Response.json({ok:true,environment:context.environment,duplicate:result.duplicate,wallet_debited:!uat,booking:{booking_id:result.booking.bookingId,status:result.booking.status,partner_reference:result.booking.partnerReference,quote_id:result.booking.quoteId,route_code:result.booking.kodeRute,amount:result.booking.amount,currency:result.booking.currency,sla:result.booking.sla,destination:result.booking.destination,created_at:result.booking.createdAt},wallet_balance:result.balance},{status:result.duplicate?200:201});
  }catch(error){const status=error?.code==='INSUFFICIENT_BALANCE'?402:error?.code==='QUOTE_NOT_FOUND'?404:error?.code==='QUOTE_FORBIDDEN'?403:apiHttpStatus(error,422);if(context)await writeApiLog(context,{status,action:context.environment==='UAT'?'BOOKING_UAT':'BOOKING',reference:error?.bookingId||null,error:error?.code||error?.message});return Response.json({ok:false,code:error?.code||'BOOKING_ERROR',message:error?.message||'Booking failed',booking_id:error?.bookingId||null,balance:error?.balance??null},{status});}
};
