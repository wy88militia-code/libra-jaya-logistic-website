...
export default async request=>{
  ...
  try{
    ...
    const uat=context.environment==='UAT';const result=uat?await createUatBookingForPartner(context.partner,context.json||{},idem):await createBookingForPartner(context.partner,context.json||{},idem,'API');await writeApiLog(context,{status:result.duplicate?200:201,action:uat?'BOOKING_UAT':'BOOKING',reference:result.booking.bookingId});
