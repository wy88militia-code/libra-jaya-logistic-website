...
export async function buildSystemHealth(){
  ...
  else if(elevationPass===elevationEligible.length)checks.push(check('ELEVATION_TERRAIN','Elevation & Terrain','PASS',`${elevationPass}/${elevationEligible.length} rute darat terverifikasi memiliki profil elevasi.`,{link:'/admin-maps-pilot',blocking:false,meta:{eligible:elevationEligible.length,pass:elevationPass,pending:0}}));
  else checks.push(check('ELEVATION_TERRAIN','Elevation & Terrain','MANUAL',`Roadmap opsional: ${elevationPass}/${elevationEligible.length} rute darat sudah memiliki profil elevasi. Sisa ${Math.max(0,elevationEligible.length-elevationPass)} belum diproses dan tidak memblokir operasional.`,{link:'/admin-maps-pilot',blocking:false,meta:{eligible:elevationEligible.length,pass:elevationPass,pending:Math.max(0,elevationEligible.length-elevationPass)}}));
  ...
  checks.push(workerCheck('PHASE1_MARKETPLACE','Phase 1 Marketplace Worker',hPhase1,{pass:8,warn:15,link:'/jlx-soetta/marketplace',missing:'MANUAL'}));
