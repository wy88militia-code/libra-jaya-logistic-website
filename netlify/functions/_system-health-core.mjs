...
export async function buildSystemHealth(){
  ...
  if(!isVendorMasterConfigured())checks.push(check('VENDOR_MASTER','Vendor Master & Cost','WARN','Credential Vendor Master belum lengkap. Operasional inti tetap dapat berjalan, tetapi expected cost/profitability vendor eksternal tidak tersedia.',{link:'/admin-vendor-master',blocking:false}));
  else if(!vendor?.rates?.length)checks.push(check('VENDOR_MASTER','Vendor Master & Cost','PASS','Sheet Vendor terkoneksi. Belum ada rate vendor eksternal aktif; pilot last-mile own-fleet memakai Modal Rute Pilot sebagai cost model.',{link:'/admin-vendor-master',blocking:false,meta:{mode:'OWN_FLEET_PILOT',vendors:vendor?.vendors?.length||0,rates:0}}));
