...
function absoluteLink(value){const link=clean(value,500);if(!link)return null;if(/^https:\/\//i.test(link))return link;if(!link.startsWith('/'))return null;const origin=String(process.env.URL||'').replace(/\/$/,'');return origin?`${origin}${link}`:link;}
...
function deliveryPolicy(notification,force=false){
  ...
  if(['ONBOARDING_SUBMITTED','ONBOARDING_APPROVED','ACTIVATION_REISSUED','INCIDENT','LOW_BALANCE','API_SECURITY_STATUS','UAT_FINAL_DECISION','PRODUCTION_ACTIVE','WEBHOOK_DEAD','WEBHOOK_QUEUE_ERROR','API_BOOKING_SPIKE','API_5XX','BACKUP_OFFSITE_FAILED','BACKUP_FAILED','RESTORE_COMPLETED'].includes(type))return {queue:true,email:true,whatsapp:true};
...
export async function queueExternalAlert(notification,{force=false}={}){
  const c=config();if(!c.anyConfigured&&!force)return null;const policy=deliveryPolicy(notification,force);if(!policy.queue)return null;const id=alertId(),createdAt=now();const record={alertId:id,notificationId:notification.notificationId||null,audience:clean(notification.audience,100),partnerId:normalizePartnerId(notification.partnerId)||null,type:clean(notification.type,80),severity:clean(notification.severity,20)||'INFO',title:clean(notification.title,180),message:clean(notification.message,1500),reference:clean(notification.reference,180)||null,link:absoluteLink(notification.link),metadata:notification.metadata&&typeof notification.metadata==='object'?notification.metadata:null,status:'PENDING',attempts:0,maxAttempts:5,nextAttemptAt:createdAt,email:initialChannel(policy.email),whatsapp:initialChannel(policy.whatsapp),createdAt,updatedAt:createdAt};await store().setJSON(`alert/${createdAt}-${id}`,record,{onlyIfNew:true});return record;
}
...
export async function queueTestAdminAlert(){const c=config();if(!c.anyConfigured)throw new Error('Email dan WhatsApp eksternal belum dikonfigurasi.');return queueExternalAlert({notificationId:null,audience:'admin',type:'SYSTEM_TEST',severity:'WARNING',title:'Test kanal alert Libra',message:'Ini adalah test email/WhatsApp dari Admin Libra. Jika pesan ini diterima, kanal eksternal berfungsi.',partnerId:null,reference:`TEST-${Date.now()}`,link:'/admin-resilience',metadata:null},{force:true});}
