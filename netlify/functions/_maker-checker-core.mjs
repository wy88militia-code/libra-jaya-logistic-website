...
async function execute(record,checker){const p=record.payload||{},actor=`${checker.username} (checker)`;switch(record.actionType){
 ...
 case 'CLAIM_SETTLEMENT':{
   if(!p.claimReference)throw new Error('Claim reference wajib untuk settlement.');const claimModule=await import('./_claim-core.mjs'),claim=await claimModule.getClaimCase(p.claimReference);
   if(!claim)throw new Error('Claim case tidak ditemukan untuk settlement.');if(claim.status!=='SETTLEMENT_PENDING'||claim.settlementApprovalRequestId!==record.requestId)throw new Error('Claim case tidak terkait dengan approval request ini atau belum SETTLEMENT_PENDING.');
...
export async function decideApproval(requestIdValue,session,{decision='APPROVE',note=''}={}){
 ...
 const choice=String(decision||'').toUpperCase();if(!['APPROVE','REJECT'].includes(choice))throw new Error('Keputusan approval tidak valid.');const stamp=now();if(choice==='REJECT'){
   const rejected={...record,status:'REJECTED',checkerUser:clean(session.username,100),checkerRole:normalizeAdminRole(session.role),decisionAt:stamp,checkerNote:clean(note,800),updatedAt:stamp};const write=await store().setJSON(key,rejected,{onlyIfMatch:entry.etag});if(!write.modified)throw new Error('Approval sedang diproses admin lain.');
   if(record.actionType==='CLAIM_SETTLEMENT'&&record.payload?.claimReference){try{const claimModule=await import('./_claim-core.mjs');await claimModule.markClaimSettlementRejected(record.payload.claimReference,id,`${session.username} (checker)`,rejected.checkerNote);}catch{}}
   return {request:rejected,before:null,after:null};
 }
