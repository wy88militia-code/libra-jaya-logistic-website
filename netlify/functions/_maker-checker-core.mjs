...
export async function decideApproval(requestIdValue,session,{decision='APPROVE',note=''}={}){
 ...
 const choice=String(decision||'').toUpperCase();if(!['APPROVE','REJECT'].includes(choice))throw new Error('Keputusan approval tidak valid.');const stamp=now();if(choice==='REJECT'){const rejected={...record,status:'REJECTED',checkerUser:clean(session.username,100),checkerRole:normalizeAdminRole(session.role),decisionAt:stamp,checkerNote:clean(note,800),updatedAt:stamp};const write=await store().setJSON(key,rejected,{onlyIfMatch:entry.etag});if(!write.modified)throw new Error('Approval sedang diproses admin lain.');return {request:rejected,before:null,after:null};}
