...
function apply(n){
 ...
 if(n===28){s.completed=true;evt('E2E_COMPLETE','Browser-local rehearsal PASS; production remains unverified.');}
 s.step=n;sessionStorage.setItem(K,JSON.stringify(s));render();}
function render(){const gp=s.profitability.grossProfit;document.getElementById('metrics').innerHTML=[['Step',s.step+' / '+STEPS.length],['Booking',s.booking.status],['Wallet',s.wallet.balance.toLocaleString('id-ID')],['Manifest',s.manifest.status],['Tracking',s.tracking.status],['Chargeable',s.weight.chargeable? s.weight.chargeable+' kg':'-'],['Gross Profit',gp? 'Rp '+gp.toLocaleString('id-ID'):'-'],['Reconcile',s.reconciliation],['Accurate',s.accurate.status],['Prod Writes',s.productionWrites]].map(x=>'<div class="metric"><small>'+x[0]+'</small><b>'+x[1]+'</b></div>').join('');document.getElementById('steps').innerHTML=STEPS.map((x,i)=>'<div class="step '+(i<s.step?'done ':'')+(i===s.step?'current':'')+'"><b>'+(i+1)+'. '+x[0]+'</b><div>'+x[1]+'</div></div>').join('');document.getElementById('state').textContent=JSON.stringify(s,null,2)}
