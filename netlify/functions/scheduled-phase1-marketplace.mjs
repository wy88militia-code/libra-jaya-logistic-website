...
export default async ()=>{
  const batches=await listPhase1MarketplaceBatches(300),active=batches.filter(b=>!['COMPLETED','CANCELLED'].includes(String(b.status||'').toUpperCase())),results=[];
  ...
  await markSystemHeartbeat('PHASE1_MARKETPLACE',{status:failed.length?'ERROR':'OK',message,metadata:{active:active.length,synced:results.length-failed.length,failed:failed.length}}).catch(()=>{});
