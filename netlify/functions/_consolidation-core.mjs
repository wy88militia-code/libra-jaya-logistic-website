...
const ACTIVE_FOR_CONSOLIDATION=new Set(['BOOKED','PICKUP_ASSIGNED','PICKED_UP','AT_ORIGIN_HUB','PAYMENT_FAILED']);
...
async function groupRecentBookings(){
  ...
  for(const booking of bookings){
    const bookingStatus=String(booking.status||'').toUpperCase();if(!ACTIVE_FOR_CONSOLIDATION.has(bookingStatus))continue;if(bookingStatus==='PAYMENT_FAILED'&&booking.minimumLoadStatus!=='TOPUP_REQUIRED')continue;
...
async function processGroup(group){
  ...
  for(const row of allocated){
    ...
    try{
      ...
      await updateBooking(b.bookingId,{serviceDate:group.serviceDate,status:'BOOKED',baseAmount:base,amount:base+adjustment,minimumLoadAdjustment:{...common,status:'CHARGED',transactionId:wallet.transactionId,chargedAt:now()},minimumLoadStatus:'CLEARED'});
