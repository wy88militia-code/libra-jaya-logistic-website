...
function normalizeDocs(input={}){const out={};for(const key of DOCUMENT_KEYS)out[key]=Boolean(input[key]===true||input[key]==='true'||input[key]==='on'||input[key]==='YES');return out;}
...
export async function createClaimCase(input={},actor='admin'){
 ...
 const existingId=await store().get(activeBookingKey(bookingId),{type:'text',consistency:'strong'});if(existingId){const existing=await getClaimCase(existingId);if(existing&&!TERMINAL.has(existing.status))throw new Error(`Booking sudah memiliki klaim aktif ${existing.claimId}.`);}
 ...
 const requestedAmount=money(input.requestedAmount);if(requestedAmount<0||requestedAmount>10000000000)throw new Error('Nilai klaim tidak valid.');
 const invoiceValue=money(input.invoiceValue),declaredValue=money(input.declaredValue);if(invoiceValue<0||declaredValue<0)throw new Error('Nilai barang tidak valid.');
 ...
 const reserved=await store().set(activeBookingKey(bookingId),claimId,{onlyIfNew:true});if(!reserved.modified&&existingId){const latest=await getClaimCase(existingId);if(latest&&!TERMINAL.has(latest.status))throw new Error(`Booking sudah memiliki klaim aktif ${existingId}.`);await store().set(activeBookingKey(bookingId),claimId);}
 const write=await store().setJSON(claimKey(claimId),claim,{onlyIfNew:true});if(!write.modified)throw new Error('Claim ID sudah digunakan.');await appendEvent(claimId,'CLAIM_OPENED',actor,{bookingId,incidentType,insuranceMode,requestedAmount});
 await updateBooking(bookingId,{hasIncident:true,claimId,claimReference:claimId,claimStatus:'OPEN',claimIncidentType:incidentType,claimUpdatedAt:createdAt});return claim;
}

export async function updateClaimCase(claimIdValue,patch={},actor='admin'){
 const claimId=clean(claimIdValue,120),entry=await getClaimCaseWithMetadata(claimId);if(!entry?.data)throw new Error('Klaim tidak ditemukan.');const current=entry.data;if(current.status==='CLOSED')throw new Error('Klaim sudah CLOSED dan dikunci.');
 const next={...current};
 if(patch.status){const status=clean(patch.status,40).toUpperCase();if(!CLAIM_STATUSES.includes(status))throw new Error('Status klaim tidak valid.');if(status!==current.status&&!TRANSITIONS[current.status]?.has(status))throw new Error(`Transisi ${current.status} → ${status} tidak diizinkan.`);next.status=status;}
 if(patch.description!==undefined)next.description=clean(patch.description,1500)||null;
 if(patch.documents)next.documents={...current.documents,...normalizeDocs(patch.documents)};
 if(patch.insurerName!==undefined)next.insurerName=clean(patch.insurerName,160)||null;
 if(patch.policyReference!==undefined)next.policyReference=clean(patch.policyReference,160)||null;
 if(patch.insurerClaimReference!==undefined)next.insurerClaimReference=clean(patch.insurerClaimReference,160)||null;
 if(patch.coverageDecision){const decision=clean(patch.coverageDecision,40).toUpperCase();if(!COVERAGE_DECISIONS.includes(decision))throw new Error('Keputusan coverage tidak valid.');next.coverageDecision=decision;}
 if(patch.coverageNote!==undefined)next.coverageNote=clean(patch.coverageNote,1000)||null;
 if(patch.approvedAmount!==undefined){const n=money(patch.approvedAmount);if(n<0||n>10000000000)throw new Error('Approved amount tidak valid.');next.approvedAmount=n;}
 if(patch.deductibleAmount!==undefined){const n=money(patch.deductibleAmount);if(n<0||n>10000000000)throw new Error('Deductible tidak valid.');next.deductibleAmount=n;}
 next.documentsComplete=documentsComplete(next);next.settlementAmount=Math.max(0,money(next.approvedAmount)-money(next.deductibleAmount));next.updatedAt=now();next.updatedBy=clean(actor,100);
 if(next.status==='APPROVED'&&next.approvedAmount<=0)throw new Error('Status APPROVED wajib approved amount > 0.');
 if(next.insuranceMode==='INSURED'&&['ELIGIBLE','PARTIAL'].includes(next.coverageDecision)&&!next.policyReference)throw new Error('Policy reference wajib sebelum coverage insured dinyatakan eligible.');
 const write=await store().setJSON(claimKey(claimId),next,{onlyIfMatch:entry.etag});if(!write.modified)throw new Error('Klaim berubah di proses lain. Refresh lalu coba lagi.');await appendEvent(claimId,'CLAIM_UPDATED',actor,{fromStatus:current.status,toStatus:next.status,coverageDecision:next.coverageDecision,documentsComplete:next.documentsComplete,approvedAmount:next.approvedAmount,deductibleAmount:next.deductibleAmount});
 if(TERMINAL.has(next.status))await store().delete(activeBookingKey(next.bookingId));await updateBooking(next.bookingId,{claimStatus:next.status,claimReference:claimId,claimCoverageDecision:next.coverageDecision,claimUpdatedAt:next.updatedAt});return next;
}

export function settlementEligibility(claim){
 const reasons=[];if(!claim)reasons.push('CLAIM_NOT_FOUND');else{if(claim.status!=='APPROVED')reasons.push('STATUS_NOT_APPROVED');if(!claim.documentsComplete)reasons.push('DOCUMENTS_INCOMPLETE');if(claim.insuranceMode==='UNKNOWN')reasons.push('INSURANCE_MODE_UNKNOWN');if(claim.insuranceMode==='INSURED'&&!['ELIGIBLE','PARTIAL'].includes(claim.coverageDecision))reasons.push('COVERAGE_NOT_APPROVED');if(claim.insuranceMode==='INSURED'&&!claim.policyReference)reasons.push('POLICY_REFERENCE_REQUIRED');if(money(claim.approvedAmount)<=0)reasons.push('APPROVED_AMOUNT_REQUIRED');if(money(claim.deductibleAmount)>money(claim.approvedAmount))reasons.push('DEDUCTIBLE_EXCEEDS_APPROVED');if(Math.max(0,money(claim.approvedAmount)-money(claim.deductibleAmount))<=0)reasons.push('SETTLEMENT_ZERO');}
 return {eligible:reasons.length===0,reasons,settlementAmount:claim?Math.max(0,money(claim.approvedAmount)-money(claim.deductibleAmount)):0};
}

export async function markClaimSettlementRequested(claimIdValue,approvalRequestId,actor='finance'){
 const claim=await getClaimCase(claimIdValue),check=settlementEligibility(claim);if(!check.eligible)throw new Error(`Klaim belum siap settlement: ${check.reasons.join(', ')}`);return updateClaimCase(claim.claimId,{status:'SETTLEMENT_PENDING'},actor).then(async next=>{const entry=await getClaimCaseWithMetadata(next.claimId);const final={...entry.data,settlementApprovalRequestId:clean(approvalRequestId,120),settlementAmount:check.settlementAmount,updatedAt:now(),updatedBy:clean(actor,100)};const write=await store().setJSON(claimKey(next.claimId),final,{onlyIfMatch:entry.etag});if(!write.modified)throw new Error('Klaim berubah saat menyimpan approval settlement.');await appendEvent(next.claimId,'SETTLEMENT_REQUESTED',actor,{approvalRequestId:final.settlementApprovalRequestId,settlementAmount:final.settlementAmount});await updateBooking(final.bookingId,{claimStatus:final.status,claimSettlementApprovalRequestId:final.settlementApprovalRequestId,claimUpdatedAt:final.updatedAt});return final;});
}
...
export async function markClaimSettlementRejected(claimIdValue,approvalRequestId,actor='checker',note=''){
 const claimId=clean(claimIdValue,120),entry=await getClaimCaseWithMetadata(claimId);if(!entry?.data)return null;const current=entry.data;if(current.status!=='SETTLEMENT_PENDING')return current;const next={...current,status:'UNDER_REVIEW',settlementApprovalRequestId:null,settlementRejectionNote:clean(note,800)||null,updatedAt:now(),updatedBy:clean(actor,100)};const write=await store().setJSON(claimKey(claimId),next,{onlyIfMatch:entry.etag});if(!write.modified)throw new Error('Klaim berubah saat reject settlement.');await appendEvent(claimId,'SETTLEMENT_REJECTED',actor,{approvalRequestId:clean(approvalRequestId,120),note:next.settlementRejectionNote});await updateBooking(next.bookingId,{claimStatus:next.status,claimUpdatedAt:next.updatedAt});return next;
}
