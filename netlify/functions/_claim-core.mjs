...
export async function createClaimCase(input={},actor='admin'){
 ...
 const incidentType=clean(input.incidentType,40).toUpperCase();if(!INCIDENT_TYPES.includes(incidentType))throw new Error('Jenis insiden tidak valid.');const insuranceMode=clean(input.insuranceMode||'UNKNOWN',30).toUpperCase();if(!INSURANCE_MODES.includes(insuranceMode))throw new Error('Mode asuransi tidak valid.');
...
export async function updateClaimCase(claimIdValue,patch={},actor='admin'){
 const claimId=clean(claimIdValue,120),entry=await getClaimCaseWithMetadata(claimId);if(!entry?.data)throw new Error('Klaim tidak ditemukan.');const current=entry.data;if(current.status==='CLOSED')throw new Error('Klaim sudah CLOSED dan dikunci.');const next={...current},settlementLocked=FINANCIAL_STATUSES.has(current.status);
 if(patch.status){const status=clean(patch.status,40).toUpperCase();if(!CLAIM_STATUSES.includes(status))throw new Error('Status klaim tidak valid.');if(current.status==='SETTLEMENT_PENDING'&&status!==current.status)throw new Error('SETTLEMENT_PENDING hanya dapat diubah oleh hasil maker-checker.');if(status!==current.status&&FINANCIAL_STATUSES.has(status))throw new Error(`Status ${status} hanya boleh diubah oleh workflow settlement maker-checker.`);if(status!==current.status&&!TRANSITIONS[current.status]?.has(status))throw new Error(`Transisi ${current.status} → ${status} tidak diizinkan.`);next.status=status;}
 if(settlementLocked){if(patch.coverageDecision&&clean(patch.coverageDecision,40).toUpperCase()!==current.coverageDecision)throw new Error('Coverage dikunci setelah settlement diajukan.');if(patch.approvedAmount!==undefined&&money(patch.approvedAmount)!==money(current.approvedAmount))throw new Error('Approved amount dikunci setelah settlement diajukan.');if(patch.deductibleAmount!==undefined&&money(patch.deductibleAmount)!==money(current.deductibleAmount))throw new Error('Deductible dikunci setelah settlement diajukan.');}
 ...
 if(patch.coverageDecision){const decision=clean(patch.coverageDecision,40).toUpperCase();if(!COVERAGE_DECISIONS.includes(decision))throw new Error('Keputusan coverage tidak valid.');next.coverageDecision=decision;}if(patch.coverageNote!==undefined)next.coverageNote=clean(patch.coverageNote,1000)||null;if(patch.approvedAmount!==undefined){const n=money(patch.approvedAmount);if(n<0||n>10000000000)throw new Error('Approved amount tidak valid.');next.approvedAmount=n;}if(patch.deductibleAmount!==undefined){const n=money(patch.deductibleAmount);if(n<0||n>10000000000)throw new Error('Deductible tidak valid.');next.deductibleAmount=n;}
...
export function settlementEligibility(claim,{allowPending=false}={}){const reasons=[];if(!claim)reasons.push('CLAIM_NOT_FOUND');else{const statusOkay=claim.status==='APPROVED'||(allowPending&&claim.status==='SETTLEMENT_PENDING');if(!statusOkay)reasons.push('STATUS_NOT_APPROVED');if(!claim.documentsComplete)reasons.push('DOCUMENTS_INCOMPLETE');if(claim.insuranceMode==='UNKNOWN')reasons.push('INSURANCE_MODE_UNKNOWN');if(claim.insuranceMode==='INSURED'&&!['ELIGIBLE','PARTIAL'].includes(claim.coverageDecision))reasons.push('COVERAGE_NOT_APPROVED');if(claim.insuranceMode==='INSURED'&&!claim.policyReference)reasons.push('POLICY_REFERENCE_REQUIRED');if(money(claim.approvedAmount)<=0)reasons.push('APPROVED_AMOUNT_REQUIRED');if(money(claim.deductibleAmount)>money(claim.approvedAmount))reasons.push('DEDUCTIBLE_EXCEEDS_APPROVED');if(Math.max(0,money(claim.approvedAmount)-money(claim.deductibleAmount))<=0)reasons.push('SETTLEMENT_ZERO');}return {eligible:reasons.length===0,reasons,settlementAmount:claim?Math.max(0,money(claim.approvedAmount)-money(claim.deductibleAmount)):0};}
