...
function zoneMap(rows=[]){const map=new Map();for(const row of rows.slice(1)){const zone=txt(row?.[7]);if(!zone||zone==='ZONA / GRUP MODAL'||zone==='AYAPO')continue;map.set(zone,{zone,minimumLoadKg:n(row?.[8]),tripsPerDay:n(row?.[9]),miscPerTrip:n(row?.[10]),fuelEfficiencyLoaded:n(row?.[11]),targetGrossMargin:n(row?.[12]),floorGrossMargin:n(row?.[13]),note:txt(row?.[14])});}return map;}
export async function getLiveFinanceRouteCosts(){
 const sheetId=process.env.MASTER_SHEET_ID||DEFAULT_SHEET_ID,t=await token(),q=new URLSearchParams({majorDimension:'ROWS',valueRenderOption:'UNFORMATTED_VALUE'});q.append('ranges',"'Modal Rute Pilot'!A24:AH200");q.append('ranges',"'Komponen Biaya'!A4:O50");
