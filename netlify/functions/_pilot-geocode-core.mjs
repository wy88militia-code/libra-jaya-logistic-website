...
async function driveRoute(latitude,longitude){
 ...
 try{const res=await fetch('https://routes.googleapis.com/directions/v2:computeRoutes',{method:'POST',signal:controller.signal,headers:{'content-type':'application/json','X-Goog-Api-Key':key,'X-Goog-FieldMask':'routes.distanceMeters,routes.duration'},body:JSON.stringify({origin:{location:{latLng:DJJ}},destination:{location:{latLng:{latitude,longitude}}},travelMode:'DRIVE',routingPreference:'TRAFFIC_UNAWARE',computeAlternativeRoutes:false,languageCode:'id-ID',units:'METRIC'})});const body=await res.json();if(!res.ok)throw new Error(body?.error?.message||`Routes HTTP ${res.status}`);const route=body.routes?.[0];if(!route?.distanceMeters)throw new Error('Routes DRIVE tidak menemukan jalur.');return {distanceKm:Number(route.distanceMeters)/1000,duration:durationText(route.duration),durationRaw:clean(route.duration)};}finally{clearTimeout(timer);}
}
...
async function geocode(row){
 ...
 if(fixed){let route=null,routeError='';try{route=await driveRoute(fixed.latitude,fixed.longitude);}catch(e){routeError=clean(e.message)||'Routes gagal';}return {...row,ok:true,latitude:fixed.latitude,longitude:fixed.longitude,address:fixed.address,source:fixed.source,status:`MANUAL PASS - TITIK KELURAHAN${route?' | ROUTES PASS':' | ROUTES GAGAL'}`,score:99,route,routeError};}
 ...
 try{const res=await fetch(url,{signal:controller.signal}),body=await res.json();if(!res.ok)throw new Error(body?.error_message||`Google Maps HTTP ${res.status}`);if(body.status!=='OK'||!body.results?.length)throw new Error(body.error_message||body.status||'Tidak ada hasil Google');const ranked=body.results.map(r=>({r,score:resultScore(r,row)})).sort((a,b)=>b.score-a.score),best=ranked[0],lat=Number(best.r.geometry?.location?.lat),lng=Number(best.r.geometry?.location?.lng);if(!Number.isFinite(lat)||!Number.isFinite(lng))throw new Error('Koordinat Google tidak valid.');if(best.score<4)throw new Error(`LOW CONFIDENCE (${best.score}) — ${best.r.formatted_address||query}`);const locationType=clean(best.r.geometry?.location_type)||'UNKNOWN',partial=best.r.partial_match?'PARTIAL MATCH':'MATCH';
  ...
  let route=null,routeError='';try{route=await driveRoute(lat,lng);}catch(e){routeError=clean(e.message)||'Routes gagal';}return {...row,ok:true,latitude:lat,longitude:lng,address:clean(best.r.formatted_address),source:'Google Maps Geocoding API + Routes API',status:`GOOGLE API PASS - ${partial} - ${locationType}${route?' | ROUTES PASS':' | ROUTES GAGAL'}`,score:best.score,route,routeError};
...
async function writeResults(token,results){
 const c=config(),date=witDate(),data=[];for(const r of results){if(r.ok){data.push({range:`'${SHEET_NAME}'!AE${r.rowNumber}:AJ${r.rowNumber}`,majorDimension:'ROWS',values:[[r.latitude,r.longitude,r.source||'Google Maps Geocoding API + Routes API',r.status,date,r.address]]});if(r.needsConfirmation)data.push({range:`'${SHEET_NAME}'!K${r.rowNumber}:L${r.rowNumber}`,majorDimension:'ROWS',values:[['','']]});else if(r.route)data.push({range:`'${SHEET_NAME}'!K${r.rowNumber}:L${r.rowNumber}`,majorDimension:'ROWS',values:[[Number(r.route.distanceKm.toFixed(1)),r.route.duration]]});}else data.push({range:`'${SHEET_NAME}'!AG${r.rowNumber}:AJ${r.rowNumber}`,majorDimension:'ROWS',values:[['Google Maps Geocoding API',`GOOGLE API GAGAL - ${clean(r.error).slice(0,180)}`,date,'']]});}
...
export async function generatePilotCoordinates(){
 ...
 return {generatedAt:new Date().toISOString(),total:results.length,success:success.length,failed:failed.length,routePass,masterSync,masterSyncError,results};
}
