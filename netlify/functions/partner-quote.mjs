...
export default async request=>{
  ...
  try{
    ...
    return Response.json({ok:true,quote:{quoteId:quote.quoteId,status:quote.status,amount:quote.amount,currency:quote.currency,weightKg:quote.weightKg,chargeableKg:quote.chargeableKg,kodeRute:quote.kodeRute,kelurahan:quote.kelurahan,distrik:quote.distrik,coverageStatus:quote.coverageStatus,skemaLayanan:quote.skemaLayanan,minimumLoadKg:quote.minimumLoadKg,sla:quote.sla,cutoffWit:quote.cutoffWit,ratePlanName:quote.ratePlanName,minimumChargeKg:quote.pricingBreakdown?.minimumChargeKg??null,expiresAt:quote.expiresAt}});
  }catch(error){
