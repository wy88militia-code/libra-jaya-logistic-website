...
export async function approveQuote(quoteId,amount,adminNote=''){const entry=await getQuoteWithMetadata(quoteId);if(!entry?.data)throw new Error('Quote tidak ditemukan.');const value=Math.trunc(Number(amount));if(!Number.isFinite(value)||value<=0)throw new Error('Nominal quote harus lebih dari nol.');const quote=entry.data;if(!['PENDING_APPROVAL','APPROVED'].includes(quote.status))throw new Error(`Quote berstatus ${quote.status} dan tidak dapat diapprove.`);const next={...quote,status:'APPROVED',amount:value,adminNote:String(adminNote||'').trim().slice(0,500),approvalSource:'ADMIN',approvedAt:now(),updatedAt:now(),expiresAt:new Date(Date.now()+APPROVED_TTL_MS).toISOString()};const result=await store().setJSON(key(quoteId),next,{onlyIfMatch:entry.etag});if(!result.modified)throw new Error('Quote berubah di proses lain. Refresh lalu coba lagi.');return next;}
