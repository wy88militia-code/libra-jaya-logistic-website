...
export default async (req: Request) => {
  ...
  const dbName = connection?.database?.alias || connection?.database?.name || connection?.database?.databaseName || 'Accurate Online';
  const authMode = connection?.mode || '-';
  const accountProbe = connection?.accountProbe?.name || connection?.accountProbe?.no || (connection?.accountProbe?.error ? 'Probe gagal' : 'OK');
  const connectionText = connection?.ok ? `API TERHUBUNG • ${dbName}` : `API GAGAL • ${connectionError || 'Tidak terhubung'}`;
  const connectionDetail = connection?.ok
    ? `Mode ${authMode} • Database ${dbName} • Probe akun ${accountProbe} • Dicek ${when(connection?.testedAt)}`
    : `Koneksi API Accurate gagal. ${connectionError || 'Periksa token, secret, database, dan jaringan.'}`;
