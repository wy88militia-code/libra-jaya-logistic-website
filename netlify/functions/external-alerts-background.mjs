...
export default async request=>{
  ...
  const results=await processDueExternalAlerts(30);const delivered=results.filter(r=>r.status==='DELIVERED').length,deadRows=results.filter(r=>r.status==='DEAD'),retry=results.filter(r=>r.status==='RETRY_PENDING').length;
