...
export default async request=>{
  ...
  const events=await listTrackingEvents(bookingId,150);const integrity=await verifyTrackingChain(bookingId);
  return Response.json({ok:true,booking:{bookingId:booking.bookingId,status:booking.status,partnerReference:booking.partnerReference,weightKg:booking.weightKg,kodeRute:booking.kodeRute,destination:booking.destination,sla:booking.sla,createdAt:booking.createdAt,bookedAt:booking.bookedAt,deliveredAt:booking.deliveredAt,receiverName:booking.receiverName,podLocked:Boolean(booking.podLocked),deliveryDistanceMeters:booking.deliveryDistanceMeters??null,podUrl:booking.podId?`/.netlify/functions/pod-media?id=${encodeURIComponent(booking.podId)}`:null},integrity,events:events.map(e=>({eventId:e.eventId,status:e.status,courierName:e.courierName,receiverName:e.receiverName,note:e.note,condition:e.condition,claimStatus:e.claimStatus,claimReference:e.claimReference,scanVerified:Boolean(e.scanVerified),destinationDistanceMeters:e.destinationDistanceMeters??null,createdAt:e.createdAt,eventHash:e.eventHash||null,podUrl:e.podId?`/.netlify/functions/pod-media?id=${encodeURIComponent(e.podId)}`:null}))},{headers:{'cache-control':'no-store'}});
};
