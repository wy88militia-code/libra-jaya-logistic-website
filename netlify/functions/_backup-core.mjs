...
const PROTECTED_STORES=['libra-partners','libra-wallets','libra-bookings','libra-quotes','libra-tracking','libra-api-uat','libra-api-onboarding','libra-rate-plans','libra-api-policies','libra-master-sync','libra-webhook-deliveries','libra-notifications','libra-admin-audit'];
const EXCLUDED_PREFIXES=new Map([
  ['libra-partners',['apikey/']],
  ['libra-wallets',[]],
]);
...
export async function createBackup({kind='MANUAL',actor='system',reason=''}={}){
  ...
  const manifest={backupId,kind:String(kind).toUpperCase(),actor:String(actor||'system').slice(0,100),reason:String(reason||'').slice(0,500),createdAt,totalEntries,totalBytes,stores};
  const payload=JSON.stringify(manifest);const checksum=crypto.createHash('sha256').update(payload).digest('hex');const record={...manifest,checksum};await backupStore().setJSON(`backup/${createdAt}-${backupId}`,record,{onlyIfNew:true});await backupStore().setJSON(`index/${backupId}`,{backupId,key:`backup/${createdAt}-${backupId}`,kind:record.kind,actor:record.actor,reason:record.reason,createdAt,totalEntries,totalBytes,checksum},{onlyIfNew:true});return record;
}
...
export async function restoreBackup(backupId,{actor='admin',reason=''}={}){
  ...
  const safety=await createBackup({kind:'PRE_RESTORE',actor,reason:`Safety snapshot sebelum restore ${backupId}. ${reason}`});let restored=0;
  for(const snapshot of backup.stores||[]){if(!PROTECTED_STORES.includes(snapshot.name))continue;const s=getStore(snapshot.name);for(const entry of snapshot.entries||[]){const bytes=Buffer.from(entry.base64,'base64');const digest=crypto.createHash('sha256').update(bytes).digest('hex');if(digest!==entry.sha256)throw new Error(`Checksum entry gagal: ${snapshot.name}/${entry.key}`);await s.set(entry.key,bytes);restored+=1;}}
  return {backupId,restored,safetyBackupId:safety.backupId,restoredAt:now()};
}

export function backupPolicy(){return {protectedStores:[...PROTECTED_STORES],scheduledRetentionDays:scheduledRetentionDays(),manualRetentionDays:manualRetentionDays(),restoreMode:'NON_DESTRUCTIVE_OVERWRITE',notes:'Restore menimpa key yang ada di snapshot tetapi tidak menghapus key baru yang dibuat setelah snapshot.'};}
