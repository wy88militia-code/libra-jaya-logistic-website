...
function scalar(v){if(v==null)return '';if(['string','number','boolean'].includes(typeof v))return v;if(typeof v==='object')return v.name??v.number??v.id??'';return '';}
function invoiceEvidence(row){
  const details=Array.isArray(row?.detailInvoice)?row.detailInvoice:[];
  return details.slice(0,20).map((d,i)=>{const inv=d?.invoice||d?.salesInvoice||d||{};return {index:i+1,id:scalar(inv.id),number:scalar(inv.number),date:scalar(inv.transDate||inv.date),amount:scalar(d?.paymentAmount??d?.amount??d?.invoiceAmount??inv?.totalAmount??inv?.amount),customer:scalar(inv?.customerName||inv?.customer),attachmentExist:inv?.attachmentExist===true};});
}
function evidence(row){
  return {
    receipt:{id:scalar(row?.id),number:scalar(row?.number),date:scalar(row?.transDate||row?.transDateView),description:scalar(row?.description),totalPayment:scalar(row?.totalPayment??row?.totalAmount??row?.paymentAmount),paymentMethod:scalar(row?.paymentMethod)},
    bank:{name:scalar(row?.bank),id:scalar(row?.bankId),journalDetailId:scalar(row?.bankJournalDetailId),transfer:scalar(row?.bankTransfer)},
    attachment:{exists:row?.attachmentExist===true,count:Number(row?.attachmentCount||0)},
    invoices:invoiceEvidence(row)
  };
}
...
async function probeDetail(id){
  ...
  return {summary:summary(row),evidence:evidence(row),attachmentSignals:inspect(row),topLevelKeys:row&&typeof row==='object'?Object.keys(row).sort():[]};
}
export default async request=>{
  ...
  const invRows=(ev?.invoices||[]).map(x=>`<tr><td>${esc(x.number||x.id||'-')}</td><td>${esc(x.date||'-')}</td><td>${esc(x.amount||'-')}</td><td>${esc(x.customer||'-')}</td><td>${x.attachmentExist?'Ya':'Tidak'}</td></tr>`).join('');
  const evidenceHtml=ev?`<section class="c"><h2>Bukti Rekonsiliasi Accurate</h2><table><tbody><tr><th>No. Penerimaan</th><td>${esc(ev.receipt.number||'-')}</td></tr><tr><th>ID</th><td>${esc(ev.receipt.id||'-')}</td></tr><tr><th>Tanggal</th><td>${esc(ev.receipt.date||'-')}</td></tr><tr><th>Deskripsi</th><td>${esc(ev.receipt.description||'-')}</td></tr><tr><th>Total pembayaran</th><td>${esc(ev.receipt.totalPayment||'-')}</td></tr><tr><th>Metode</th><td>${esc(ev.receipt.paymentMethod||'-')}</td></tr><tr><th>Bank/Akun</th><td>${esc(ev.bank.name||'-')}</td></tr><tr><th>Bank ID</th><td>${esc(ev.bank.id||'-')}</td></tr><tr><th>Bank Journal Detail ID</th><td>${esc(ev.bank.journalDetailId||'-')}</td></tr><tr><th>Bank Transfer</th><td>${esc(ev.bank.transfer||'-')}</td></tr><tr><th>Bukti transfer/attachment</th><td>${ev.attachment.exists?`Tersedia (${ev.attachment.count})`:'Tidak tersedia'}</td></tr></tbody></table><h3>Faktur yang terkait dengan penerimaan</h3><table><thead><tr><th>No. Faktur</th><th>Tanggal</th><th>Nilai terkait</th><th>Pelanggan</th><th>Attachment faktur</th></tr></thead><tbody>${invRows||'<tr><td colspan="5">Tidak ada detailInvoice yang dikembalikan.</td></tr>'}</tbody></table><p><b>Catatan:</b> seluruh data di atas dibaca langsung dari detail Penerimaan Penjualan Accurate. Halaman ini tidak membuat pembayaran, jurnal, atau perubahan data.</p></section>`:'';
  return new Response(`<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Tes Attachment Accurate</title><style>body{font-family:system-ui;background:#f3f6f9;color:#10243d;margin:0}.w{max-width:980px;margin:auto;padding:24px}.c{background:white;border:1px solid #dce6ef;border-radius:16px;padding:18px;margin:12px 0}select,button{padding:11px;border-radius:9px}button{background:#075182;color:white;border:0;font-weight:800}table{width:100%;border-collapse:collapse;font-size:12px}th,td{padding:9px;border-bottom:1px solid #eee;text-align:left;vertical-align:top}.ok{color:#176b37}.bad{color:#9e2621}code{white-space:pre-wrap;word-break:break-word}</style></head><body><main class="w"><h1>Tes Baca Attachment Accurate</h1><p>READ-ONLY • Penerimaan Penjualan • tidak download, tidak posting, tidak mengubah Accurate.</p><section class="c"><form method="get"><label>Pilih Penerimaan Penjualan</label><br><select name="id" required><option value="">— pilih —</option>${options}</select> <button type="submit">Baca Detail</button></form>${error?`<p class="bad"><b>ERROR:</b> ${esc(error)}</p>`:''}</section>${evidenceHtml}${result?`<section class="c"><h2>${result.attachmentSignals.length?'<span class="ok">Referensi attachment/file terdeteksi</span>':'<span class="bad">Belum ada field attachment/file pada response detail</span>'}</h2><p><b>Field detail yang dikembalikan:</b> ${esc(result.topLevelKeys.join(', '))}</p><table><thead><tr><th>Path</th><th>Tipe</th><th>Preview metadata</th></tr></thead><tbody>${signals||'<tr><td colspan="3">Tidak ditemukan key attachment/document/file/media/image/photo/url.</td></tr>'}</tbody></table><h3>Ringkasan transaksi</h3><code>${esc(JSON.stringify(result.summary,null,2))}</code></section>`:''}<p><a href="/admin-ai-accurate">← AI Agent Accurate</a></p></main></body></html>`,{headers:{'content-type':'text/html; charset=utf-8','cache-control':'no-store','x-frame-options':'DENY','content-security-policy':"default-src 'none'; style-src 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'"}});
};
