...
2. Configure the Payment Session webhook URL to:
   `https://YOUR-DOMAIN/.netlify/functions/xendit-partner-webhook`
3. Use the Xendit webhook verification token as `XENDIT_WEBHOOK_TOKEN` in Netlify.
4. The partner top-up function creates a Xendit Payment Session in `PAYMENT_LINK` mode.
5. Wallet credit occurs only after a verified `payment_session.completed` webhook with matching session ID and amount.
...
- Test login, top-up, Xendit redirect, webhook, wallet credit, and booking debit in Test Mode.
